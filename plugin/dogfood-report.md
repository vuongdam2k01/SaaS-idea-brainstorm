...
> Ý tưởng: `dogfood/proposal-draft/` (đã dời khỏi `ideas/` — conflicts C1) — "AI soạn proposal từ notes discovery-call cho solo consultant". Chế độ: Analysis, auto_continue, **founder mô phỏng (dán nhãn tường minh — mechanics test)**. Phạm vi chạy: new-idea → stage 0 trọn → stage 1 (agents thật) → F signing ceremony → gatekeeper gate F. Điều kiện môi trường: `--plugin-dir` child-session bị chặn auth trong session lồng nhau → dogfood logic thực thi theo đúng skill files; hook chạy qua invocation trực tiếp trên artifact thật.
