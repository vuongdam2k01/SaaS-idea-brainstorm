...
**The contract has to be legible where the code is written.** A coding agent arrives knowing none of it. **In the same repository as `ideas/<slug>/` this is already handled and nothing needs installing:** the plugin's SessionStart hook carries the build contract (the two layers, amendment-log-first, the product-vs-technical line), a PostToolUse hook injects the anchor mechanism and id vocabulary the first time a session opens a spec file, `guard-thresholds.js` blocks an edit to a locked artifact and names `amend-blueprint`, and `/saas-idea-brainstorm:spec` resolves any id to its defining file and section — built from the artifacts on demand, so an amendment is live the moment it is written. Because that path generates nothing, it has almost no surface against the other plugins a build repo carries: no `.claude/rules/` written, no `.claude/settings.json` touched, no project skills added (plugin skills are namespaced and cannot collide), nothing to regenerate and nothing to delete on uninstall. The one friction worth knowing in advance: **a formatter or lint plugin that rewrites `**/*.md` will hit the locked artifacts and be blocked by `guard-thresholds.js`.** That is correct behaviour — locked bytes are the record of what was decided — but it reads as a mysterious failure, so exclude `ideas/` in that tool's configuration.

**Only when the code lives in a separate repository** does anything get generated: `handoff-to-build` (`scripts/build-handoff.js`) ships a hashed read-only `docs/product/` copy plus `AGENTS.md`, `CLAUDE.md` importing it, path-scoped `.claude/rules/product-spec/`, `/product-spec` + `/product-spec-gap`, and a SessionStart hook that verifies the copy against its source. It injects awareness only — no architecture, stack or workflow — and paraphrases nothing. That kit must be regenerated after every amendment (`--check` reports staleness, cheap enough for CI), which is the standing cost of the two-repo shape and a good reason to prefer one repo when there is a choice.
