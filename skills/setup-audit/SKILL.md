...
1. Write per-capability objects into `state.json.capabilities` per state-schema: `{status, rung, provider, verified_at, probe, note}` — `rung` is one of exactly three values `enhanced-auto | baseline-auto | handoff`; never write `handoff-only` or `simulate`.
