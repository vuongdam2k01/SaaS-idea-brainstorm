...
and frozen before this repo existed**. They live in `{{SPEC_ROOT}}/`. You do not have to infer
them from the code, and you must not re-decide them.

`{{SPEC_ROOT}}/` is not documentation *about* the product. It is the **specification the code
owes**. Where code and spec disagree, the spec is right until it is amended through the process
in [Rule 1](#rule-1--the-spec-files-are-read-only).
...
| # | file | why |
|---|---|---|
| 1 | `{{SPEC_ROOT}}/blueprint/amendment-log.md` | **First, always.** Current truth = locked blueprint **+** amendments. Skipping it means building against a superseded decision. Absent = no amendments yet. |
| 2 | `{{SPEC_ROOT}}/{{PACK}}/mvp-spec.md` | Scope boundary: what is in, and the **cut list** of what was deliberately left out |
| 3 | `{{SPEC_ROOT}}/blueprint/blueprint-overview.md` | Index of every spec file, the event dictionary, and the decision register |
| 4 | the feature spec you are about to implement | `{{SPEC_ROOT}}/blueprint/feature-specs/fs-NN-*.md` |

Full file list with one line of purpose each: `{{READ_ORDER}}`.
Machine-readable map from any id to its file and section: `{{INDEX}}`
...
|---|---|---|---|
| **Pack** (`{{SPEC_ROOT}}/{{PACK}}/`) | mvp-spec, tech-design, definition-of-done, charter… | *what / why / where the boundary is* | not from this repo — scope change goes back to the validation pipeline |
| **Blueprint** (`{{SPEC_ROOT}}/blueprint/`) | feature specs, data schema, UX, API, integrations, NFRs, test plan, build plan | *exactly how, at the product level* | amendment only (Rule 1) |
...
1. `{{INDEX}}` → `ids["AC-03-2"]` gives `{file, anchor, label}`
2. the `/{{SKILL_SPEC}}` skill in this repo: `/spec AC-03-2`
3. grep the id inside `{{SPEC_ROOT}}/`
...
## Rule 1 — the spec files are read-only
...
Use `/{{SKILL_GAP}}` in this repo to work out which case you have and to draft the request.
...
3. **Product question, in scope** → stop and raise it. `/{{SKILL_GAP}}` drafts the amendment request.
...
`{{SPEC_ROOT}}/blueprint/test-plan.md` states which scenarios are mandatory regardless of feature
work.

## Rule 6 — the cut list is a boundary, not a backlog

`{{SPEC_ROOT}}/{{PACK}}/mvp-spec.md` lists what was deliberately excluded. Those items are not
...
**{{AMENDMENTS_THROUGH}}**. `{{INDEX}}` holds a SHA-256 for every copied file.
...
product fact lives in `{{SPEC_ROOT}}/` and only there. If you want to know what the product does,
