...
- **Cold-start test — run it HERE, before the full gate check.** Copy `mvp-pack/` alone into a temporary clean directory, then spawn `coldstart-tester` (Agent tool) pointed at the copy; testing in place would miss exactly the broken `../` references the copy exposes. **Persist the run before anything else** (a remembered PASS is not evidence — method-rules §1): create the manifest over the copied set (`artifact-manifest.js create … --out private/manifest-coldstart-l1-<date>-<NN>.json`), then write `coldstart-l1-<date>-<NN>.md` at the **idea root** (never inside the pack — writing it in would change the very set it certifies; next unused NN, never overwrite, FAIL runs persisted too) containing the verdict verbatim, the manifest citation `path@sha256`, and the per-file sha table inlined from the manifest. Cite the report as `path@sha256` from `audit-trail.md`'s LOCK section, so a clone that receives only pack + audit trail still has evidence the bar was met. Any in-scope open question → fix the pack and re-run (new NN). LOCK's formal predicate requires the **latest** report to be a PASS with hashes matching the current pack, so invoking the full gate check before this has run guarantees a pointless FAIL.
...
**After gate-check reports PASS**, the validation pipeline ends: the MVP Pack is issued and frozen.
**Build does not start here.** One stage remains between the pack and the first line of product
code: **stage 6 — implementation blueprint** (`stage-6-blueprint` skill, gate BP), which turns the
pack's what/why/boundary into the full pre-implementation doc set — feature specs, field-level
schema, UX spec, interface contracts, NFRs, test plan, build plan — with the founder deciding every
product question the pack left open. Invoke it next (or offer it now if the founder wants a break;
`state.blueprint` and session-start will keep pointing at it). The build-phase reference at
`${CLAUDE_SKILL_DIR}/../../process/build-and-launch.md` (on other runtimes:
`process/build-and-launch.md` at the plugin root) applies after gate BP passes, with pack +
blueprint as the two-layer contract.
