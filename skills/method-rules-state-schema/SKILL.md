...
- **Durability**: every `state.json` mutation **MUST go through** `node "${CLAUDE_SKILL_DIR}/../../scripts/state-write.js" <path>` with the new content on stdin (validates shape, keeps `.bak`, atomic rename — `${CLAUDE_PLUGIN_ROOT}` is NOT substituted in skill content, only `${CLAUDE_SKILL_DIR}` is). This was "prefer" until dogfood run #2 measured compliance at 0/21 direct writes and one real truncation — a rule with 0% observed compliance is not a rule, so the writer is now mandatory and the `guard-thresholds` hook additionally blocks any direct edit that drops a load-bearing top-level key. Only if Node is genuinely unavailable: read-modify-write with a self-check re-read. On an agent runtime that doesn't substitute `${CLAUDE_SKILL_DIR}` in skill content (e.g. Codex), resolve the plugin root yourself — locate this file's own directory and go up two levels — before invoking the script. Artifacts are ground truth; state is an index that must be rebuildable from them + decision-log.
