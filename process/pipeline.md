...
→ **Build starts with pack + blueprint as the two-layer contract** — see [build-and-launch.md](build-and-launch.md). Before the first build session, run `handoff-to-build`: it generates the awareness files the coding tools load unprompted — path-scoped `.claude/rules/product-spec/`, the `/product-spec` + `/product-spec-gap` skills, and an id index — either into a separate build repo (`--to`, adding `AGENTS.md` / `CLAUDE.md`, a hashed read-only copy and a SessionStart briefing) or `--in-place` when one repo holds the idea workspace and the code. Passing BP proves a fresh session *could* implement from the set; the kit is what makes a session aware the set exists instead of inferring the product from the tree.
