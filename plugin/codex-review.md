...
- NEW: `skills/method-rules-gate-contracts/SKILL.md` (machine-readable gate contract + 3-tier pack predicate: Validated / Hypothesis / **Pre-feasibility**), `skills/switch-mode/`, `scripts/state-write.js`, `tests/hook-tests.js` (16 tests), `LICENSE`.
...
- the `method-rules-artifact-schema` skill: added `invariant-change` to the decision-log type enum; added a full `unvalidated-build-decision.md` artifact (frontmatter, template, ownership note that gate-check never writes it) and a gate-check FAIL-branch pointer to it.
