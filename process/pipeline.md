...
→ **Build starts with pack + blueprint as the two-layer contract** — see [build-and-launch.md](build-and-launch.md). Passing BP proves a fresh session *could* implement from the set; something still has to make that session aware the set exists instead of inferring the product from the tree. In the same repo the plugin does it natively — SessionStart carries the build contract, a PostToolUse hook injects the id vocabulary on first contact with a spec file, and `/saas-idea-brainstorm:spec` resolves an id on demand. Only for a **separate** build repository is anything generated: `handoff-to-build` ships a hashed read-only copy plus `AGENTS.md` / `CLAUDE.md`, path-scoped rules and an id index, for a repo that may not have this plugin installed.
