...
→ **Build starts with pack + blueprint as the two-layer contract** — see [build-and-launch.md](build-and-launch.md). Before the first build session, run `handoff-to-build`: it generates, into the build repository, the awareness files the coding tools load unprompted (`AGENTS.md` / `CLAUDE.md`, path-scoped `.claude/rules/`, `/spec` + `/spec-gap`, a SessionStart briefing, and a hashed read-only `docs/product/` copy with an id index). Passing BP proves a fresh session *could* implement from the set; the kit is what makes a session in another repo aware the set exists instead of inferring the product from the tree.
