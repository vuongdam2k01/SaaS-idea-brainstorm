...
**First, make the contract legible where the code will live.** The pack and blueprint are read by a coding agent that arrives knowing none of it. The `handoff-to-build` skill (`scripts/build-handoff.js`) generates the files those tools load unprompted: path-scoped `.claude/rules/product-spec/` (id vocabulary on the spec root, the product-vs-technical boundary on source paths, acceptance-criteria-as-oracle on test paths), the `/product-spec` and `/product-spec-gap` skills, and `.claude/product-spec/spec-index.json` mapping every id to its **defining** file and section. Two modes: `--to <repo>` when the code lives in a separate repository (adds `AGENTS.md`, `CLAUDE.md` importing it, a hashed read-only `docs/product/` copy, and a SessionStart hook that verifies hashes and alarms on drift), or `--in-place` when one repo holds both — which copies nothing, because a copy inside the same tree would be an editable twin of a frozen file (the pipeline's hooks guard only `ideas/**`). It injects awareness only — no architecture, stack or workflow — and paraphrases nothing, so it cannot drift from the artifacts. Everything lands under two deletable namespaces and every generated rule disclaims style, commit and release process, so it coexists with the other plugins a build repo carries. Regenerate after every amendment; `--check` reports staleness and is cheap enough for CI.
