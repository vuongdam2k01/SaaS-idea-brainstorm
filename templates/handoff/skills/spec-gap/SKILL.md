...
The specification in `{{SPEC_ROOT}}/` was written to be complete enough to implement from without
...
1. `{{SPEC_ROOT}}/blueprint/amendment-log.md` — may already answer it, and overrides the locked files
2. the feature spec's `bp:edge-cases`, `bp:states`, `bp:fields`, `bp:open-decisions`
3. `{{SPEC_ROOT}}/blueprint/blueprint-overview.md` → `bp:decisions` (the `DR-n` register of decisions
...
4. `{{SPEC_ROOT}}/blueprint/interaction-map.md` — cross-feature rules, invariants, job semantics
5. `{{SPEC_ROOT}}/blueprint/data-schema.md`, `nfr-spec.md`, `api-contract.md`, `integration-specs.md`
6. `{{SPEC_ROOT}}/{{PACK}}/mvp-spec.md` — it may be out of scope rather than unspecified

```bash
node ${CLAUDE_PROJECT_DIR}/{{LOOKUP}} --grep <keyword>
```
...
Check `{{SPEC_ROOT}}/{{PACK}}/mvp-spec.md`. If the behaviour is on the cut list or outside the locked
...
Never edit `{{SPEC_ROOT}}/` to record the answer locally. Those files are byte-frozen and hashed;
