...
3. **Finalize bytes**: render every published file in FINAL form with `publication_status: locked` already set. Then hash with the **shared helper** — the same module and the same bytes gate verdicts use, so there is one integrity implementation and no second dialect:
   ...
   (idea-relative paths only — the helper rejects traversal and absolute forms; it sorts paths, hashes raw bytes, records the algorithm, and hashes the manifest itself.) The JSON the helper writes (`manifest-<id>.json`) is **canonical**; `reconcile/<id>/manifest-<id>.md` is a human-readable **view** of it and says so on its first line — it carries the same hashes + the manifest's own sha256 + charter high-water mark/hash + drift items with dispositions + observation index, and if the two ever disagree the JSON is right. Two authoritative-looking manifests would be two integrity stories. Verify predecessors with `artifact-manifest.js verify` at step 4.2 and again here. No byte changes after this point.
