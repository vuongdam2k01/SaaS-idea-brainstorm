...
`AC-NN-n` criteria in `{{SPEC_ROOT}}/blueprint/feature-specs/` are written binary — given/when/then
...
`/{{SKILL_GAP}}` → amendment in the source workspace. Not an edit here.
...
`{{SPEC_ROOT}}/blueprint/test-plan.md` is the authority. It binds every definition-of-done item,
...
`JOB-n` rows in `{{SPEC_ROOT}}/blueprint/interaction-map.md` specify cancellation, second submit,
